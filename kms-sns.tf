##############################################
# KMS Key for SNS Topic (ewec-prod-alerts)
##############################################
...
data "aws_iam_policy_document" "sns_kms_policy" {
  ...
  statement {
    ...
    condition {
      ...
      values   = ["arn:aws:sns:me-central-1:035838344560:ewec-prod-alerts"]
    }
  }
}

resource "aws_kms_key" "sns" {
  description         = "KMS key for SNS topic ewec-prod-alerts (prod)"
...
resource "aws_kms_alias" "sns" {
  name          = "alias/sns/ewec-prod-alerts"
