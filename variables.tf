...
variable "eks_ng_ewec_prod_desired" {
  type = number
}

variable "eks_ng_ewec_prod_min" {
  type = number
}

variable "eks_ng_ewec_prod_max" {
  type = number
}
...
variable "ingress_host" {
  description = "Hostname for the Ingress rule (e.g. app.prod.example.com). Leave empty for wildcard host."
  type        = string
  default     = ""
}
...
variable "alb_unhealthy_threshold" {
  description = "Unhealthy target count threshold for ALB"
  type        = number
}

# === EKS (ContainerInsights) ===
variable "eks_container_restarts_threshold" {
  description = "Total container restarts across cluster during the evaluation window"
  type        = number
}

# === KMS ===
variable "kms_disabled_key_threshold" {
  description = "DisabledKey metric threshold; 1 means alert if key is disabled"
  type        = number
}

variable "kms_failed_key_rotation_threshold" {
  description = "KeyRotationEnabled metric threshold; <1 alerts (rotation disabled)"
  type        = number
}

# === NAT Gateway ===
variable "nat_error_port_allocation_threshold" {
  description = "ErrorPortAllocation sum threshold per period"
  type        = number
}

# === RDS ===
variable "rds_freeable_mb" {
  description = "FreeableMemory lower threshold (MiB)"
  type        = number
}

variable "rds_db_connections" {
  description = "DatabaseConnections upper threshold"
  type        = number
}

variable "rds_read_lat_ms" {
  description = "ReadLatency p95 threshold (ms)"
  type        = number
}

variable "rds_write_lat_ms" {
  description = "WriteLatency p95 threshold (ms)"
  type        = number
}

variable "rds_replica_lag" {
  description = "ReplicaLag threshold (seconds) — only applies if replicas exist"
  type        = number
}

# === Redis ===
variable "redis_curr_conn" {
  description = "CurrConnections upper threshold"
  type        = number
}

variable "redis_swap_usage_mb" {
  description = "SwapUsage upper threshold (MiB)"
  type        = number
}
