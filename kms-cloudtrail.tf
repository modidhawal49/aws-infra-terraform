...
data "aws_iam_policy_document" "cloudtrail_kms_policy" {
  ...
  statement {
    ...
    # Scope to this account’s trail ARN (name must match your aws_cloudtrail.this name)
    condition {
      test     = "StringEquals"
      variable = "kms:EncryptionContext:aws:cloudtrail:arn"
      values = [
        "arn:${data.aws_partition.current.partition}:cloudtrail:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:trail/${var.name_prefix}-trail"
      ]
    }
  }
}
